...
Prepara os dados brutos de rastreamento antes da análise comparativa:
converte os vídeos originais, recorta por animal, exporta as coordenadas do
DeepLabCut (.h5 -> .txt) com filtro de confiança, e organiza os arquivos
brutos do TopScan (offset de mosaico + eventos de exploração mesclados).

TopScan sai daqui ainda BRUTO — sem filtragem de outliers, suavização ou
calibração espacial. Essas etapas rodam uma única vez, no notebook de
Análise, aplicadas simetricamente a TopScan e DeepLabCut (ver Methods do
artigo, "Coordinate preprocessing").
"""
import glob
import os
import re
...
# ├── VIDEOS/
# │   ├── originais_mpg/        <- vídeos brutos multi-animal (.mpg)
# │   ├── convertidos_mp4/      <- saída da conversão (.mp4)
# │   └── recortados/           <- saída do recorte por animal
# ├── DEEPLABCUT/
# │   ├── h5_exportado/         <- arquivos .h5 exportados pelo DLC
# │   └── trajetoria_txt_dlc/   <- TXTs gerados por este notebook
# └── TOPSCAN/
#     ├── TXT original e XLSX original/  <- pares brutos TopScan (.TXT + .xlsx)
#     └── TXT novo/                      <- TXTs organizados por este notebook
# ============================================================================
RAIZ_DRIVE = "/content/drive/MyDrive/validacao_topscan"


# ==============================================================================
# 1. Conversão de vídeos (.MPG -> .MP4)
# ==============================================================================
def converter_videos(pasta_entrada, pasta_saida, prefixo_arquivo="M"):
    ...
    for nome_arquivo in tqdm(arquivos, desc="Convertendo vídeos", unit="vídeo"):
        ...
        try:
            ...
            print(f"Erro ao ler {nome_arquivo}: {e.stderr.decode('utf-8')}")
            continue
...
# ==============================================================================
# 2. Recorte de vídeos multi-animal
#
# Cada vídeo bruto contém 3 arenas lado a lado (mosaico). O nome do arquivo
# segue o padrão "M1--AA-BB-CC.mp4": "M1" identifica o mosaico e "AA-BB-CC"
# os três animais, na ordem em que aparecem nos quadrantes do vídeo.
# ==============================================================================
def recortar_video_por_animal(caminho_video, pasta_saida, largura_barra=35):
    ...
    try:
        ...
        print(f"Nome fora do padrão ignorado: {nome_arquivo}")
        return
    ...
    # Quadrantes: 1=top-left, 2=top-right, 3=bottom-left
    quadrantes = [
    ...
    while True:
        ...
        for idx, escritor in escritores.items():
            ...
            recorte[:, -largura_barra:] = cor_barra  # oculta o timestamp sobreposto pelo TopScan
            escritor.write(recorte)

    cap.release()
    for escritor in escritores.values():
        escritor.release()
    print(f"Finalizado: {nome_arquivo} -> {', '.join(ids[:len(escritores)])}")


# ==============================================================================
# 3. Exportação DeepLabCut (.h5 -> .txt) com filtro de confiança
# ==============================================================================
...
def exportar_deeplabcut_para_txt(pasta_h5, pasta_saida, bodypart, limiar_likelihood=0.8):
    """Extrai X/Y de um bodypart de cada .h5 filtrado do DeepLabCut, descarta
    frames com likelihood abaixo do limiar (viram NaN) e grava um .txt por
    animal. Separa automaticamente os modelos held-out (leave-one-mosaic-out)
    dos modelos completos, pelo nome do arquivo exportado pelo DLC."""
    os.makedirs(pasta_saida, exist_ok=True)
    ...
    for caminho_h5 in caminhos_h5:
        ...
        try:
            ...
            print(f"Bodypart '{bodypart}' ausente em: {arquivo}")
...
# ==============================================================================
# 4. Organização do TopScan bruto (offset de mosaico + eventos de exploração)
# ==============================================================================
def _ler_eventos(eventos_path):
    """Lê a tabela de eventos (From Frame, To Frame, Event) de .xlsx (formato
    original do TopScan, cabeçalho na 2ª linha) ou .csv."""
...
def organizar_topscan(txt_path, eventos_path, saida_path):
    print(f"Processando {os.path.basename(txt_path)}...")
    ...
    # Cada gravação contém até 3 arenas lado a lado (mosaico); corrige o offset
    # de acordo com o quadrante do animal, identificado pelo nome do arquivo
    # (ex.: "AA-BB-CC_2_..." -> segundo animal do mosaico, quadrante top-right).
    ...
    print(f"  Salvo: {os.path.basename(saida_path)}")
...
def organizar_topscan_em_lote(pasta_entrada, pasta_saida):
    ...
    for nome_txt in sorted(f for f in os.listdir(pasta_entrada) if f.upper().endswith(".TXT")):
        ...
        if os.path.exists(csv_path):
            ...
            print(f"Eventos ausentes para {nome_txt} -- pulado.")
