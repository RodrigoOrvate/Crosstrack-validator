...
if sys.platform == 'win32':
    # Console do Windows (cp1252) não decodifica os emojis usados em vários prints
    # do notebook original (✅, 🎉, etc.) — força UTF-8 na saída padrão.
...
import matplotlib
matplotlib.use('Agg')  # execução local não-interativa: gera arquivos, não janelas
...
# Raiz do repositório, usada para todos os caminhos locais abaixo.
...
RATOS_MODO_CONSERVADOR  = {'M5-PP', 'M5-II'} #Utilize para ratos que possuem trajetória pobre e não estão plotando corretamente
NORMALIZAR_VISUAL_60X60 = True

# Geração de vídeo
SISTEMAS_PARA_GERAR_VIDEO = ['topscan', 'dlc']  # quais sistemas geram vídeo anotado
RATOS_PARA_GERAR_VIDEO    = ['M1-AA']                  # [] = todos os ratos comuns
...
# Obter lista de vídeos disponíveis na pasta (vídeos originais não fazem parte
# dos dados de exemplo do repositório -- pasta pode não existir).
...
class CalibradorSerial:
    ...
    def finalizar(self):
        ...
        print("A variável 'ROI_TEMPLATES_DINAMICO' está pronta para o pipeline.")
        print("="*40)
...
def _rotular_painel_com_borda(fig, ax, letra, fontsize=20, pad=0.010):
    """
    Desenha uma borda ao redor do eixo `ax` e a letra do painel (ex.: 'A') em
    uma caixinha branca no canto superior esquerdo -- mesmo estilo usado nas
    Figs. 4/5. Função genérica para reaproveitar em Fig. 7, S8, S11, etc.

    Usa `ax.get_tightbbox()` em vez de `ax.get_position()` porque este
    último cobre só a área de plotagem, não rótulos de eixo, ticks ou
    legendas que ultrapassam essa área -- o que faria a borda cortar parte
    da figura em painéis com legendas/labels maiores.
    """
    fig.canvas.draw()
...
def extrair_id_rato_topscan(nome_arquivo):
    """
    TopScan: {COD1}-{COD2}-{COD3}_{N}_rest.TXT → retorna 'M#-COD' (Mosaico adaptado).
    """
    partes = nome_arquivo.split('_')
    if len(partes) >= 2:
        try:
            ...
            if 0 <= idx < len(codigos):
                ...
                # Mapa interno para padronizar com o DLC
                mapa_mosaico = {
...
def _normalizar_visual_independente(df, arena_w=60.0, arena_h=60.0):
    """
    Normalização visual INDEPENDENTE (cada sistema preenche 0-60x0-60 sozinho).
    Usada nos painéis A e B da Fig. 4 -- ao contrário de
    `_normalizar_visual_60x60` (escala compartilhada), evita que o TopScan
    apareça comprimido quando sua amplitude nativa é menor que a do DLC.
    """
...
def extrair_id_rato_dlc(nome_arquivo):
    """Lê o nome do arquivo e extrai o ID e o Bodypart usando Regex.

    Aceita tanto o padrão do dataset completo do artigo
    ("ID_M1-AA_trajetoria_body_full.txt", com mosaico) quanto o padrão
    simplificado usado nos dados de exemplo do repositório
    ("ID_AA_trajetoria_body.txt", sem mosaico), para o script rodar tanto
    com o dataset completo quanto com os exemplos fornecidos em
    Data/Examples/.
    """
...
def extrair_id_rato_topscan(nome_arquivo):
    """Lê o nome do arquivo TopScan e extrai o ID."""
    match_id = re.search(r'(M[1-6]-[A-Z]{2})', nome_arquivo, re.IGNORECASE)
    if match_id:
...
def carregar_dados():
    ...
    if os.path.exists(pasta_dlc):
        for arq in os.listdir(pasta_dlc):
            ...
            if rid and bp and bp in BODYPARTS_ANALISE:
                # Guarda o caminho com o prefixo 'full/' só quando essa subpasta existe
                arquivos_dlc[bp][rid] = os.path.join('full', arq) if usa_subpasta_full else arq
    else:
        print(f"ATENÇÃO: A pasta {pasta_dlc} não foi encontrada!")
    ...
    print("--- DIAGNÓSTICO ---")
    print(f"Ratos encontrados no DLC (Pasta Full): {len(ratos_dlc)}")
    print(f"Ratos encontrados no TopScan: {len(ratos_topscan)}")
    print(f"INTERSEÇÃO (Ratos válidos pareados): {len(ratos_comuns)}")
    print("-------------------")
    ...
    print(f"Ratos válidos processados ({len(ratos_comuns)}): {sorted(ratos_comuns)}")
...
# ============================================================
# INSPEÇÃO DOS FRAMES COM MAIOR ERRO POSICIONAL
...
def calcular_exploracao_comportamental(ratos_comuns, arquivos_topscan, arquivos_dlc, rois_definidas):
    ...
    for rato_id in ratos_comuns:
        ...
        # --- DIAGNÓSTICO 1: O que está faltando? ---
        if not all([info_ts, info_dlc, rois]):
            print(f"\n[{rato_id}] IGNORADO na Etapa de Gráficos. Motivo:")
            print(f"  - TopScan TXT: {'Encontrado' if info_ts else 'FALTOU'}")
            print(f"  - DLC TXT (ponto '{BODYPART_LEGENDA}'): {'Encontrado' if info_dlc else 'FALTOU'}")
            print(f"  - ROIs configuradas: {'Encontradas' if rois else 'FALTOU'}")
            continue

        # --- TopScan ---
        df_ts = carregar_dados_topscan_txt(os.path.join(DIRETORIO_TOPSCAN_TXT, info_ts['arquivo_txt']))

        # --- DIAGNÓSTICO 2: Coluna Areas ---
        if df_ts is None or 'Areas' not in df_ts.columns:
            print(f"[{rato_id}] IGNORADO: Coluna 'Areas' não encontrada no arquivo TopScan.")
            continue
        ...
        if df_dlc is None:
            print(f"[{rato_id}] IGNORADO: Falha ao ler o TXT do DLC.")
            continue

        df_dlc = df_dlc[df_dlc['Frame'] > IGNORAR_FRAMES_INICIAIS].copy()

        # ... (O RESTANTE DO SEU CÓDIGO CONTINUA IGUAL DAQUI PRA BAIXO) ...
        df_dlc = adicionar_coluna_tempo(df_dlc, 'Frame', FPS_VIDEO)
...
def gerar_figura_brokenbar_erro_estado(
    ...
    if all(c in _piv_estado.columns for c in ordem_estado) and len(_piv_estado) >= 3:
        ...
        print(f"  Friedman (erro por estado comportamental): chi2({len(ordem_estado)-1}) = {_chi2_estado:.2f}, "
...
print("\n=== Figura: Bland-Altman + Índice de Discriminação ===")

# -----------------------------
# Dados do Índice de Discriminação
# -----------------------------
di_ts_list, di_dlc_list, ratos_di = [], [], []
...
if 'df_metricas' not in dir() or not isinstance(df_metricas, pd.DataFrame) or df_metricas.empty:
    ...
    if len(_df_pca_in) < 3:
        ...
        print("\n  Tabela S4 — comparação Alta vs. Baixa Concordância (8 métricas, Mann-Whitney):")
        print(df_tabela_s4.to_string(index=False))
        # (DIRETORIO_OUTPUT_EXPORTACAO só é definido mais adiante no script — usa RAIZ
        # diretamente aqui para não depender da ordem de execução das células.)
        ...
        # ── 7. Biplot PC1 × PC2 (painel C, centralizado embaixo) ────────────────
        ax = axes[2]
        ...
        # ── 8. Loadings Plot (PC1 e PC2) (painel B, topo-direita) ──────────────
        ax = axes[1]
...
if _n_v < 3:
    ...
    # ==============================================================
    # BLOCO 1 — CONCORDÂNCIA INTEROBSERVADOR  EXP1 × EXP2
    # ==============================================================
    ...
    # ------------------------------------------------------------------
    # Painel B (era D) — B-A Observer 1 − Observer 2
    # ------------------------------------------------------------------
...
print("\n=== ETAPA 5: Análise Leave-One-Mosaic-Out (Held-out) ===")
...
plt.show()
print(f"Figure S4 exportada: {_path_s4}")
...
plt.show()
print(f"Figure S7 exportada: {_path_s7}")

# 1. Função para carregar as coordenadas
def carregar_coordenadas(rato_id, bodypart, tipo='full'):
    if tipo == 'full':
        ...
        # Descobre qual mosaico o rato pertence
...
for rato_id in sorted(ratos_comuns):
    ...
    if any(df is None for df in [nose_full, nose_held, body_full, body_held]):
        print(f"  [Aviso] Dados faltantes para {rato_id}. Ignorando na Etapa 5.")
        continue

    # Calcula Erros Posicionais (Full vs Held-out) em pixels
    # (Como ambos são DLC, podemos comparar direto em px ou aplicar a escala da Etapa 1)
    ...
    # Recalcula Exploração para o Held-out
    rois = rois_definidas.get(rato_id)
...
if dados_s5:
    ...
    print(f"✅ Tabela S5 exportada: {_path_s5}")
    ...
    # A. Erro de pose nos frames nunca vistos (Nose vs Mid-body)
    ...
    # B. DLC-HELDOUT x TopScan para DI
    ...
    # C. DLC-HELDOUT x Manual para DI
    ...
    # D. Bland-Altman DLC-FULL x DLC-HELDOUT para DI
    ...
    print(f"  FigS11 A -- Kruskal-Wallis (RMSE por fold excluido): H = {_h_rmse_s11:.2f}, "
    ...
    print(f"  FigS11 B -- Kruskal-Wallis (tempo de exploracao por fold excluido): H = {_h_exp_s11:.2f}, "
    ...
    # Salvando direto na pasta suplementar para não engatilhar o erro do dicionário restrito
    ...
    # Linhas conectando o mesmo animal
    ...
    # Salvando direto na pasta suplementar
    ...
    # Salva apenas a principal na rotina de exportação restrita
    exportadas_sup = _exportar_figura_registrada('figura7_heldout', fig7)

    print("✅ Análise Held-out finalizada e exportada com sucesso!")

    # 1. Ensina ao pipeline os nomes finais dos arquivos para exportação
    ...
    # Guardamos as trajetórias, tempos e as estatísticas demoradas
    variaveis_para_salvar = {
        'resultados_cm': resultados_cm,
        'rois_definidas': rois_definidas,
        'tempos_salvos': tempos_salvos,
        'df_metricas': df_metricas  # <--- O tesouro que custa 1 hora!
    }

    with open(caminho_backup, 'wb') as f:
        pickle.dump(variaveis_para_salvar, f)

    print("✅ Backup TOTAL salvo! Você não precisará calcular o DTW novamente.")
    ...
    print("AVISO: nenhum dado held-out (LOMO) disponível -- pulando Fig. 7, FigS11, FigS12 e Tabela S5.")